...
description: "GASでスプレッドシートをデータベースとして使うDiscord Botの構築手順を解説。データの登録・照会・排他制御まで、コピペで動く完全コードとトラブルシューティング表付き。約60分で無料のDiscordデータ管理Bot（スプレッドシートDB）を構築できます。"
...
faq:
  - question: "スプレッドシートをデータベースとして使うのは安全ですか？"
    answer: "小規模なデータ管理には十分安全です。機密情報を扱う場合は、スプレッドシートのアクセス権限を適切に設定し共有範囲を限定してください。スクリプトプロパティでIDを管理することで、コードからの情報漏洩も防げます。"
  - question: "スプレッドシートDBの同時書き込みは大丈夫ですか？"
    answer: "GASのLockServiceを使えば排他制御が可能です。この記事のコードではデータ追加時にスクリプトロックを取得し、同時書き込みによるデータ破損を防いでいます。高頻度アクセスが継続する場合は、専用データベースへの移行を検討してください。"
...
relatedArticles: ["discord-bot/discord-bot-overview", "discord-bot/discord-bot-gas", "gas/gas-spreadsheet-automation"]
draft: false
---

> この記事は[Discord Bot業務活用ガイド](/articles/discord-bot/discord-bot-overview)の実装編です。
> Discord Botの基礎知識やGASについて知りたい方は、先に[GAS入門ガイド](/articles/gas/gas-basics)をご覧ください。

この記事では、GASでスプレッドシートをデータベースとして使うDiscord Botを構築します。約60分で完成する手順を、全コード付きで解説します。

DiscordスプレッドシートDBとは、Googleスプレッドシートをデータベース代わりに使い、Discord Botからデータの登録・照会をおこなう仕組みです。専用のデータベースサーバーを用意せずに、無料でDiscordデータ管理機能を構築できます。
...
スクリプトプロパティを使うことで、コードへのハードコードを防げます。
...
LockServiceを使って、複数ユーザーの同時書き込みでもデータが壊れない安全なデータ追加を実装します。
